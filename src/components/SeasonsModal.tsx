...
import { X, Clock, Star, ChevronDown } from "lucide-react";
...
const SeasonsModal = ({ seriesId, seasons, onClose }: SeasonsModalProps) => {
...
const EpisodeCard = ({ episode }: { episode: TMDBEpisode }) => (
  <div className="flex gap-4 p-3 rounded-2xl bg-white/[0.03] border border-white/5 hover:bg-white/[0.06] hover:border-white/10 transition-all group">
    {/* Thumbnail */}
    <div className="flex-shrink-0 w-28 sm:w-36 aspect-video rounded-xl overflow-hidden bg-muted">
      {episode.still_path ? (
        <img
